...
def algo(context):
    ...
    fin = get_fundamentals(table='trading_derivative_indicator', symbols=not_suspended[0],
                           start_date=last_day, end_date=last_day, fields='PB,NEGOTIABLEMV', df=True)  # 获取P/B和市值数据
    for record in not_suspended[1:]:
        r_new = get_fundamentals(table='trading_derivative_indicator', symbols=record,
                               start_date=last_day, end_date=last_day, fields='PB,NEGOTIABLEMV', df=True)  # 获取P/B和市值数据
        fin = pd.concat([fin,r_new],axis=0)
...
if __name__ == '__main__':
    ...
    '''
    run(strategy_id='6582950c-bc92-11ed-9b9c-00fff1ca1445',
        filename='test1.py',
        mode=MODE_BACKTEST,
        token='82ecee4230a8bb420ae916009ed890fa568338e7',
        backtest_start_time='2017-07-01 08:00:00',
        backtest_end_time='2018-07-01 16:00:00',
